...
namespace Kooboo.CMS.Content.Persistence.SqlServer.Tests
{
    ...
    public class QueryTests
    {
        ...
        public void WhereIn()
        {
            var news = newsFolder.CreateQuery().WhereEquals("Published", null).OrderBy("UtcCreationDate").WhereIn("UUID", new[] { newsContent.UUID, news2.UUID }).ToArray();
            Assert.AreEqual(newsContent.UUID, news.First().UUID);
            Assert.AreEqual(news2.UUID, news.Skip(1).First().UUID);
