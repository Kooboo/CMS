...
namespace Kooboo.CMS.Content.Persistence.Sqlce.Tests
{
    ...
    public class QueryTests
    {
        ...
        public QueryTests()
        {
            ...
            newsSchema.AddColumn(new Column() { Name = "title", DataType = Data.DataType.String, Length = 256 });
        ...
        {
            var content = new TextFolder(repository, "news", null) { SchemaName = newsSchema.Name.ToLower(), CategoryFolders = new[] { categoryFolder.FullName }, IsDummy = false }.CreateQuery().WhereEquals("UUID", newsContent.UUID).FirstOrDefault();
            Assert.IsNotNull(content);
        ...
        {
            var contents = repository.ExecuteQuery(string.Format("SELECT * FROM [{0}]", newsSchema.GetTableName()));
            Assert.AreEqual(3, contents.Count());

            var count = repository.ExecuteScalar(string.Format("SELECT COUNT(*) FROM [{0}] WHERE UUID=@uuid", newsSchema.GetTableName()),
        ...
        [TestMethod]
        public void Test_TextContentProvider_QueryCategories()
        {
            var categories = provider.QueryCategories(newsContent);
