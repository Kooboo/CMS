...
(function (ctx, $, undefined) {
    ...
    zindexCenter.prototype = {
        ...
        queryWinZMax: function (win) {
            ...
            $(win.document.body).find('*').each(function () {
                cur = parseInt($(this).css('z-index'), 10);
        ...
        restoreOld: function (el) {
            var o = $(el), zold = o.attr('KOOBOOZOLD');
            if (!zold) {
                return o;
            } else {
                return o.css('z-index', (parseInt(zold, 10) || 0)).removeAttr('KOOBOOZOLD');
            }
        },
        ...
        setTop: function (el, set) {
            ...
            // apply
            this.restoreOld(el);
            var o = $(el), old = parseInt(o.css('z-index'), 10) || 0;
            return o.attr('KOOBOOZOLD', old).css('z-index', old + max);
        },
    ...
    if (ctx) {
        ctx.zindexCenter = singleton;
        ctx.zTop = function (el) { return singleton.setTop(el); }
        ctx.zOld = function (el) { return singleton.restoreOld(el); }
    }
