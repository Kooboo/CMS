    ...
    widgetMask.prototype = {
        ...
        findEndNode: function () {
            ...
            var end, find = function (next) {
                while (next) {
                    if (next.tagName === 'VAR' && $(next).attr('end') === '') {
                        end = next;
                        break;
                    }
        ...
        mask: function () {
            this.unmask();
            // create
            this.maskEl = $('<var style="position:absolute;"></var>').appendTo(yardi.cacheCon || 'body');
            // css
            this.maskEl.addClass(this.cssClass);
            this.maskPosition();
            yardi.zTop(this.maskEl);
            // event
            this.registerResize();
        },
        ...
        region: function () {
            ...
            markup.push('</var>');
            this.regionCon = $(markup.join('')).appendTo(yardi.cacheCon || 'body');
            // css
            this.regionPosition();
            var cls = this.cssClass;
            this.regionCon.children().each(function () {
                yardi.zTop($(this)).addClass(cls);
            });
            // event
            this.registerResize();
        },
        ...
        border: function () {
            ...
            markup.push('</var>');
            this.borderCon = $(markup.join('')).appendTo(yardi.cacheCon || 'body');
            // gen border color
            ...
            this.borderCon.children().each(function () {
                yardi.zTop($(this)).css({ borderColor: borderColor });
            });
            // event
            this.registerResize();
        },
