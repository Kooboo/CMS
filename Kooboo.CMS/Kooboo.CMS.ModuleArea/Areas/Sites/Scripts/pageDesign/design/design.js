    ...
    yardi.extend(htmlContent, content, {
        ...
        processHtml: function () {
            ...
            if ($.browser.msie && $.browser.version < 9) {
                elem.innerHTML = '<span>temp</span>' + this.el.attr('Html');
                elem.removeChild(elem.firstChild);
            } else {
                elem.innerHTML = this.el.attr('Html');
            }
            this.processScript(elem);
        },
        processScript: function (con) {
            var exeScripts = [];
            var scripts = con.getElementsByTagName('script');
            $.each(scripts, function () {
                if (!this.isExecuded) { exeScripts.push(this); }
            });
            if (!exeScripts.length) { return; }
            // hack
            var write = document.write, writeln = document.writeln;
            var elem = exeScripts[0], parent = elem.parentNode, next = elem.nextSibling;
            ...
            var self = this, complete = function () {
                ...
                self.processScript(con);
            };
            if (elem.src) {
                $.ajax({ url: elem.src, async: false, dataType: 'script', complete: complete });
            } else {
                $.globalEval(elem.text || elem.textContent || elem.innerHTML || ''); complete();
            }
        }
    });
