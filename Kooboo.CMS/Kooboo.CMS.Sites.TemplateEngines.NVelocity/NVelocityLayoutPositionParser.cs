...
{
    public class NVelocityLayoutPositionParser : ILayoutPositionParser
    {
        ...
        public IHtmlString RegisterClientRemovePosition()
        {
            string script = "function removePosition(name,body){" +
                "var reg = /\\$Html.FrontHtml\\(\\s*\\).Position\\(\\s*\"\\w+\"\\s*\\)/g,nameReg = new RegExp('\"'+name+'\"');  return body.replace(reg, function (a, b) { if (nameReg.test(a)) { return ''; } else return a; });" +
                " }";
