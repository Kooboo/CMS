...
{
    [Kooboo.CMS.Web2.Authorizations.Authorization(AreaName = "Contents", Group = "", Name = "Content", Order = 1)]
    [ValidateInput(false)]
        ...
        public virtual ActionResult ImageCrop()
        {
            ...
            data.RunWithTry((resultData) =>
            {
                if (Request.Files.Count > 0 && Request.Files[0].ContentLength > 0)
                {
                    ...
                    resultData.Model = new { ImageUrl = Url.Content(fileUrl), PreviewUrl = this.Url.Action("ResizeImage", "Resource", new { url = fileUrl, area = "", width = 600, height = 400, preserverAspectRatio = true, quality = 80 }) };
                }
                else
                {
